...
## Pre-processing to check if variables are read as the right data type

```{r}
sapply(Merged_SNP_Maize[, 1:10], mode)
...
```{r}
### Use the sapply to create a function "i" and repeat it for each chromosome. This section first sorts the code and then arranges the postions within in ascending order. Later it replaces missing data with "?" and finally creates a new file totalling 20 files. The file can later be opened for review. One could change the file name in the read.table command to to view specific files.
...
data_maize_inc_1 <- read.table("./increasing_maize/increasing_maize1")
View(data_maize_inc_1)
```

## Maize - 10 files for each chromosome with decreasing position and missing values replaced by "-"

```{r}
### Similar to the previous chunk of code except this code replaces "?/?" with "-" and also arranges the position in a decreasing order. Again, one could change the file name in the read.table command to to view specific files.
...
data_maize_dec_1 <- read.table("./decreasing_maize/decreasing_maize1")
View(data_maize_dec_1)
```
...
data_teosinte_inc_1 <- read.table("./increasing_teosinte/increasing_teosinte1")
View(data_teosinte_inc_1)
```
...
data_teosinte_dec_1 <- read.table("./decreasing_teosinte/decreasing_teosinte1")
View(data_teosinte_dec_1)
```
...
### Chromosome 1 has the most SNPs, while Chromosome 10 has the least.
```
...
### When comparing Groups we observe an overall lower amount of Homozygosity in Teosinte individuals over Maize counterparts. This is reflected when visualizing cumulative counts between the two Samples.
```

## Missing Data

```{r}
NAData <- SNP_Homo_Hetero %>% 
  mutate(
    Missing_Values = if_else(
...
ggplot(data = NAData) + 
  geom_bar(mapping = aes(x = Missing_Values, fill = Group)) + 
  xlab(label = "Missing or Not") + ylab(label = "Count") +
  ggtitle("Missing Data in Maize + Teosinte") + facet_grid(type ~ .)

ggplot(data = NAData) +
  geom_bar(mapping = aes(x = type, fill = Missing_Values)) + 
  xlab(label = "Sample") + ylab(label = "Count") +
  ggtitle("Missing Data in Maize vs Teosinte")

### Although there is not a lot missing from the Teosinte sample,  considering the SNPs it is quite concerning.  
```
...
### Although graphs presented above show the SNP distribution in absolute estimates, this pie chart shows the proportion contributed by each Group within a Sample type.
```
